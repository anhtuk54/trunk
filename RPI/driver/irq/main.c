...
static struct platform_driver 	g_platform_driver = 
{
	...
	.remove	= __exit_p(iodev_iomem_remove)
};
...
int iodev_iomem_probe(struct platform_device * pxPlatfDev) 
{
	...
	else 
	{
		...
		g_io_stData[0].m_pxDevice = &pxPlatfDev->dev;
	}
	...
	// Get IRQ line number
	//g_io_stData[0].m_irq_line = irq_of_parse_and_map(pxPlatfDev->dev.of_node,0);
	g_io_stData[0].m_irq_line = gpio_to_irq(17);
...
int iodev_init_gpio(void) {
	// setup gpio port
	BIT_CLR((unsigned long*)(g_io_stData[0].m_pvirt_base),12);
	BIT_CLR((unsigned long*)(g_io_stData[0].m_pvirt_base),13);
	BIT_CLR((unsigned long*)(g_io_stData[0].m_pvirt_base),14);
	return 0;
}
...
int iodev_irq_probe(void) 
{
	...
	LOG_NOTI("IRQ line: %d", g_io_stData[0].m_irq_line);
	
	return g_io_stData[0].m_irq_line;
}
...
void iodev_irq_init(void)
{
	...
	// Probing IRQ line
	_irq_line = iodev_irq_probe();
	
	// Request IRQ line
	_errno = request_irq(_irq_line \
						,iodev_irq_handler \
						,IRQF_TRIGGER_HIGH \
						,DEF_IO_MODULE_NAME \
						,DEF_IO_MODULE_NAME);
...
int __init io_init(void) {
	...
	_errno = platform_driver_probe(&g_platform_driver, iodev_iomem_probe);
	if (_errno) 
	{
		return -1;
	}
